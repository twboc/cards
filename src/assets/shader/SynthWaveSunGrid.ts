...
float sun(vec2 uv, float battery) {
    ...
    float cut =
        3.0 * sin((uv.y + iTime * 0.2 * (battery + 0.02)) * 100.0) +
...
half4 main(vec2 fragCoord){
    ...
    if (uv.y < -0.2){
        ...
        vec2 sceneUV = uv;
        sceneUV.y -= battery*1.1 - 0.51;

        vec2 sunUV = sceneUV + vec2(0.75,0.2);
        ...
        float fujiVal = sdTrapezoid(
            sceneUV + vec2(-0.75,0.5),
            1.75 + pow(sceneUV.y*sceneUV.y,2.1),
            0.2,
            0.5
        );

        float waveVal = sceneUV.y + sin(sceneUV.x*20.0+iTime*2.0)*0.05 + 0.2;
        float waveWidth = smoothstep(0.0,0.01,waveVal);

        col = mix(col,
            mix(vec3(0.0,0.0,0.25),vec3(1.0,0.0,0.5),fujiD),
            step(fujiVal,0.0));

        col = mix(col,vec3(1.0,0.5,1.0),
            waveWidth * step(fujiVal,0.0));
        ...
        col += mix(col,
            mix(vec3(1.0,0.12,0.8),
                vec3(0.0,0.0,0.2),
                clamp(sceneUV.y*3.5+3.0,0.0,1.0)),
            step(0.0,fujiVal));

        vec2 cloudUV = sceneUV;
        ...
        col = mix(col,vec3(0.0,0.0,0.2),
            1.0 - smoothstep(0.0749,0.075,cloudVal));
